...
// we will braing in the openzepplin ERC721 NFT functionality
...
contract KBMarket is ReentrancyGuard {
  ...
  arrays need to kwno the length */
  ...
  // we are deploying to matic the API is the same so you can use ether the same as mat
  ...
  function createMarketSale(
    ...
    public payable nonReentrant {
      ...
      uint tokenId = idToMarketToken[itemId].toeknId;
      require(msg.valuee == price, 'Please submit the asking price in order to continue');
    ...
    function fetchMarketTokens() public view returns(MarketToken[] memory) {
      ...
      // looping over the number of items created (if number has not been sold populate the)
    ...
    function fetchMyNFTs() public view returns (MarketToken[] memory) {
      ...
      // second loop to loop through the abount you have purchased with itemcount
